...
#define PIXELATION_FILE
    ...
    vec2 ComputeTexelOffset(vec2 uv, vec4 texelSize) {
        ...
        // 2. Calculate how the texture coords vary over fragment space.
        ...
        // Cutout foliage can collapse one derivative axis. Inverting its
        // singular matrix yields NaN/Inf, which Mesa/Zink propagates as bright
        // leaking pixels. Keep the original fragment instead of snapping it.
        float determinant = dUVdS.x * dUVdT.y - dUVdS.y * dUVdT.x;
        float derivativeScale = max(
            max(abs(dUVdS.x), abs(dUVdS.y)),
            max(abs(dUVdT.x), abs(dUVdT.y))
        );
        // A merely non-zero determinant is not sufficient: near-singular UV
        // gradients can produce a huge screen offset, which Mesa propagates
        // into isolated coloured pixels in the gbuffer.
        if (abs(determinant) <= max(1.0e-8, derivativeScale * derivativeScale * 1.0e-4)) return vec2(0.0);

        // 3. Invert the texture delta from fragment delta matrix. Where the magic happens.
        mat2x2 dSTdUV = mat2x2(dUVdT[1], -dUVdT[0], -dUVdS[1], dUVdS[0]) * (1.0 / determinant);

        // 4. Convert the texture delta to fragment delta.
        vec2 dST = dUV * dSTdUV;
        if (any(notEqual(dST, dST)) || any(greaterThan(abs(dST), vec2(8.0)))) return vec2(0.0);
        return dST;
    }

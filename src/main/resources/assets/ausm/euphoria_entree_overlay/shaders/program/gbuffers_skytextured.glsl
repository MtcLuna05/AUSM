...
uniform int ausmSkyDetailKind;
uniform vec3 ausmAstralConstellationColor;
uniform vec3 ausmAstralTierColor;
...
vec3 AusmConstellationTint(vec3 fallbackTint) {
    #if ASTRAL_CONSTELLATION_STYLE == 1
        vec3 selectedTint = ausmAstralTierColor;
    #else
        vec3 selectedTint = ausmAstralConstellationColor;
    #endif
    float tintMax = max(max(selectedTint.r, selectedTint.g), selectedTint.b);
    if (tintMax <= 0.001) selectedTint = fallbackTint;
    tintMax = max(max(selectedTint.r, selectedTint.g), selectedTint.b);
    return mix(vec3(1.0), selectedTint / max(tintMax, 0.001), 0.82);
}

float AusmConstellationNodeMask(vec2 localCoord) {
    vec2 p = abs(localCoord * 2.0 - 1.0);
    float p2 = dot(p, p);
    float core = 1.0 - smoothstep(0.0064, 0.1156, p2);
    float horizontalRay = (1.0 - smoothstep(0.03, 0.12, p.y)) * (1.0 - smoothstep(0.18, 0.92, p.x));
    float verticalRay = (1.0 - smoothstep(0.03, 0.12, p.x)) * (1.0 - smoothstep(0.18, 0.92, p.y));
    float softHalo = (1.0 - smoothstep(0.0324, 0.6084, p2)) * 0.35;
    return clamp(max(max(core, horizontalRay), verticalRay) + softHalo, 0.0, 1.0);
}

vec4 AusmAstralConstellationColor(vec4 color, vec2 localCoord, float skyHeight, vec3 vertexTint, bool nodeTexture) {
    float sourceLum = max(max(color.r, color.g), color.b);
    vec3 tint = AusmConstellationTint(vertexTint);
    if (nodeTexture) {
        float nodeMask = AusmConstellationNodeMask(localCoord);
        if (nodeMask <= 0.01) discard;
        color.rgb = tint * (0.65 + nodeMask * 0.85);
        color.a = nodeMask * max(0.55, skyHeight);
    } else {
        color.rgb = tint * max(sourceLum, 0.45);
        color.a *= 0.72 * max(0.55, skyHeight);
    }
    return color;
}
#endif
...
void main() {
    ...
    vec4 averageSkyColor = vec4(0);
        ...
        if (ausmSkyDetailKind == 5) {
            #if ASTRAL_CONSTELLATION_STYLE == 0
                if (color.a <= 0.001) discard;
            #else
                bool ausmAstralNodeTexture = abs(tSize.x - 32.0) < 0.5 && abs(tSize.y - 32.0) < 0.5;
                color = AusmAstralConstellationColor(color, texCoord, 1.0, glColor.rgb, ausmAstralNodeTexture);
            #endif
        ...
        bool vanillaCelestialTexture = (abs(tSize.x - 32.0) < 0.5 && abs(tSize.y - 32.0) < 0.5)
                                   || (abs(tSize.x - 128.0) < 0.5 && abs(tSize.y - 64.0) < 0.5);
        if (ausmSimpleVoidWorld == 1 && vanillaCelestialTexture) discard;
...
vec4 AusmConstellationVertexPosition() {
    vec4 viewVertex = gl_ModelViewMatrix * gl_Vertex;
    if (ausmSkyDetailKind == 5) {
        vec3 rotationAxis = normalize(gbufferModelView[0].xyz);
        float angle = -sunPathRotation * 0.01745329251994;
        float sine = sin(angle);
        float cosine = cos(angle);
        vec3 position = viewVertex.xyz;
        viewVertex.xyz = position * cosine
                + cross(rotationAxis, position) * sine
                + rotationAxis * dot(rotationAxis, position) * (1.0 - cosine);
    }
    return gl_ProjectionMatrix * viewVertex;
}

//Program//
void main() {
    gl_Position = AusmConstellationVertexPosition();

...
#endif
#include "/lib/atmospherics/ausmOwnedSky.glsl"
...
void main() {
    ...
    // The procedural AUSM canvas contains the Botania/Astral Void World
    // details. Normal Overworld routes retain Entree's standard sky and
    // celestial rendering assembled above.
    if (ausmSimpleVoidWorld > 0) {
        vec4 ausmScreenPos = vec4(gl_FragCoord.xy / vec2(viewWidth, viewHeight), gl_FragCoord.z, 1.0);
        vec4 ausmViewPos = gbufferProjectionInverse * (ausmScreenPos * 2.0 - 1.0);
        ausmViewPos /= ausmViewPos.w;
        vec3 ausmStarlessSky;
        color.rgb = AusmOwnedSkyColor(normalize(ausmViewPos.xyz), upVec, sunVec, ausmStarlessSky);

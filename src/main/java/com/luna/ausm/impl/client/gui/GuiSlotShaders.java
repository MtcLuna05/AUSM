...
public class GuiSlotShaders {
    ...
    public void handleMouseInput() {
        handleMouseInput(Mouse.getEventDWheel());
    }

    public void handleMouseInput(int wheel) {
        if (wheel != 0) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks, boolean keyboardFocused) {
        ...
        for (int index = first; index < last; index++) {
            ...
            int rowBottom = y + slotHeight - 4;
            if (selected) {
                drawRect(panelLeft + 14, y, panelRight - 14, rowBottom, 0x88000000);
                drawRect(panelLeft + 14, y, panelRight - 14, y + 1, 0xFFBFC7D1);
                drawRect(panelLeft + 14, rowBottom - 1, panelRight - 14, rowBottom, 0xFFBFC7D1);
                drawRect(panelLeft + 14, y, panelLeft + 15, rowBottom, 0xFFBFC7D1);
                drawRect(panelRight - 15, y, panelRight - 14, rowBottom, 0xFFBFC7D1);
            } else if (hovered) {
                drawRect(panelLeft + 14, y, panelRight - 14, y + 1, 0xFF71849A);
                drawRect(panelLeft + 14, rowBottom - 1, panelRight - 14, rowBottom, 0xFF71849A);
                drawRect(panelLeft + 14, y, panelLeft + 15, rowBottom, 0xFF71849A);
                drawRect(panelRight - 15, y, panelRight - 14, rowBottom, 0xFF71849A);
            }
            int color = selected ? 0xFFFFFF : 0xC8CED6;
            int textLeft = panelLeft + 28;
            int availableTextWidth = Math.max(1, panelRight - 22 - textLeft);
            String display = label;
            while (MinecraftReflectionCompat.fontStringWidth(MinecraftReflectionCompat.fontRenderer(mc), display)
                    > availableTextWidth && display.length() > 3) {
                display = display.substring(0, display.length() - 4) + "...";
            }
            MinecraftReflectionCompat.fontDrawString(MinecraftReflectionCompat.fontRenderer(mc),
                    display, textLeft, y + 6, color);
        }

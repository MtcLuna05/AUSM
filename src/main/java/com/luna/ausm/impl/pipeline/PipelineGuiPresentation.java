...
import static com.luna.ausm.impl.pipeline.PipelineGlState.restoreVanillaClientRenderState;
...
import static com.luna.ausm.impl.pipeline.PipelineProbeLimits.MAX_GUI_RECOVERED_BACKGROUND_LOGS;
...
abstract class PipelineDeferredPassOrchestration0 extends PipelineChunkUpdateTracking {
    ...
    /**
     * Restores the shadered world immediately before a third-party HUD cache
     * alpha-composites its contents into Minecraft's framebuffer.
     *
     * <p>Gnetum owns a transparent HUD framebuffer and normally expects the
     * Minecraft framebuffer to retain the world it is drawing over. That
     * assumption is not valid after AUSM has finished its pipeline: a later
     * cache pass can leave the target cleared even though the final pipeline
     * image was valid. The per-frame direct-presentation snapshot is the
     * authoritative world image at this point, so restore it only when it was
     * captured for the current pipeline frame.</p>
     *
     * @return {@code true} when a current world snapshot was restored
     */
    public boolean restoreCurrentWorldForExternalHudComposite() {
        if (!isPipelineActive || externalWorldFramebufferTarget != null || self().isRenderingBetterPortalsNestedView()) {
            return false;
        }
        Minecraft mc = MinecraftReflectionCompat.minecraft();
        Framebuffer target = mc != null ? MinecraftReflectionCompat.minecraftFramebuffer(mc) : null;
        return self().refreshMinecraftFramebufferFromDirectPresentationTexture(target, false);
    }

    public void observePresentationBeforeWorldRendering(Minecraft minecraft) {
        if (!isPipelineActive || externalWorldFramebufferTarget != null || minecraft == null || !directPresentationValid) {
            return;
        }
        Framebuffer target = MinecraftReflectionCompat.minecraftFramebuffer(minecraft);
        PipelinePresentationDivergenceProbe.observe("previous-window", minecraft, pipelineFrameId, target,
                directPresentationFbo, directPresentationTexture, directPresentationWidth, directPresentationHeight);
    }
    ...
    public void prepareBypassedGuiScreenDrawState() {
        renderingGui = false;
        guiRenderDepth = 0;
        // Gnetum has already bound its transparent HUD cache. Its framebuffer
        // mixin redirects a Minecraft-framebuffer bind back into that cache,
        // so the normal direct GUI initializer would perturb the cache while
        // it is being composited. Restore only GL state here; retain the
        // caller's framebuffer binding and viewport.
        MinecraftReflectionCompat.glUseProgram(0);
        TextureBinder.restoreDefaultTextureUnit();
    ...
    public void beginGuiScreenRendering() {
        self().beginOwnedGuiScreenRendering();
    }

    public void beginOwnedGuiScreenRendering() {
        self().beginOwnedGuiRendering(true);
    }

    public void beginOwnedHudRendering() {
        self().beginOwnedGuiRendering(false);
    }

    protected void beginOwnedGuiRendering(boolean screen) {
        if (externalWorldFramebufferTarget != null || self().isRenderingBetterPortalsNestedView()) {
            return;
        }

        boolean outermostGui = guiRenderDepth == 0;
        boolean preserveCompletedGui = outermostGui && guiTargetContentFrame == pipelineFrameId;
        if (outermostGui && isPipelineActive && worldFrameActive) {
            self().renderDeferredNativeBloomIfNeeded();
            self().blitWorldFramebufferToMinecraft();
        }
        guiRenderDepth++;
        ...
        Minecraft mc = MinecraftReflectionCompat.minecraft();
        Framebuffer target = mc != null ? MinecraftReflectionCompat.minecraftFramebuffer(mc) : null;
        if (target != null && isPipelineActive && !preserveCompletedGui) {
            self().refreshMinecraftFramebufferFromDirectPresentationTexture(target, true);
        }
        self().prepareDirectGuiScreenRenderingState(false);
        renderingGui = true;
        if (screen && outermostGui) {
            self().prepareVanillaGuiScreenOverlayState();
        } else {
    ...
    public void finishOwnedGuiRendering() {
    ...
    protected boolean refreshMinecraftFramebufferFromDirectPresentationTexture(Framebuffer target, boolean allowStaleForGui) {
        ...
        // A paused screen has no new world presentation to snapshot. Reuse the
        // last complete world image for that screen so its translucent layers do
        // not accumulate into the Minecraft framebuffer from one GUI frame to
        // the next. Normal HUD/world presentation still requires this frame's
        // snapshot.
        if (directPresentationFrame == Long.MIN_VALUE || age < 0L || (!allowStaleForGui && age != 0L)) {
            return false;
        }
    ...
    public boolean presentFramebufferDirectly(Framebuffer target, int width, int height) {
        ...
        if (mc == null
                || target == null
                || target != MinecraftReflectionCompat.minecraftFramebuffer(mc)
                || MinecraftReflectionCompat.framebufferObject(target) <= 0
                || width <= 0
                || height <= 0) {
            return false;
        }
        ...
        PipelinePresentationDivergenceProbe.observeWindowPresentation(
                mc,
                pipelineFrameId,
                width,
                height,
        ...
        PipelineFrameLayerCapture.mirrorWindowPresentation(width, height);
        PipelineFrameLayerCapture.captureFinalWindowPresentation(pipelineFrameId, width, height);

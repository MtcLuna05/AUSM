...
abstract class NothiriumShadowVisibleLayerRendering extends NothiriumShadowSelection {
    ...
    protected Iterable<?> boundedMainTerrainCompileScan(BlockRenderLayer layer, Iterable<?> chunks) {
        ...
        NothiriumShadowRenderer.RotatingShadowSelectionChunkList window = mainTerrainCompileScanWindows[layer.ordinal()];
        window.reset(source, start, count);
    ...
    protected NothiriumShadowRenderer.ChunkOrigin chunkOrigin(NothiriumShadowRenderer.Reflection reflection, Object chunk) throws ReflectiveOperationException {
        // The direct accessor is the normal path. Its short-lived origin is
        // scalar-replaced by the JVM, whereas an IdentityHashMap lookup was
        // visible in the shadow hot loop. Cache only reflective fallbacks.
        ...
        if (shadowSelectionActive) {
            NothiriumShadowRenderer.ChunkOrigin cached = shadowChunkOrigins.get(chunk);
            if (cached != null) {
                return cached;
            }
            NothiriumShadowRenderer.ChunkOrigin origin = new NothiriumShadowRenderer.ChunkOrigin(
                    reflection.getX(chunk), reflection.getY(chunk), reflection.getZ(chunk));

...
import static com.luna.ausm.impl.pipeline.PipelineProbeLimits.MAX_HAND_GBUFFER_PROBE_LOGS;
...
abstract class PipelineRuntimeDiagnosticsState7 extends PipelineRuntimeDiagnosticsState6 {
    ...
    public void finishGuiEntityPreviewRenderState() {
        ...
        guiEntityPreviewStateDepth--;
        GL11.glPopAttrib();
        self().restoreGuiAlphaCompositionAfterEntityPreview();
        if (!isPipelineActive) {
            return;
        }
        MinecraftReflectionCompat.glUseProgram(0);
    ...
    /**
     * GuiInventory's entity preview can leave the real driver alpha/blend state
     * disabled while GlStateManager still believes both capabilities are enabled.
     * Empty equipment-slot sprites are drawn immediately afterwards, so their
     * transparent pixels otherwise become opaque black. Repair the driver first,
     * then mirror the same state through GlStateManager to keep its cache aligned.
     */
    protected void restoreGuiAlphaCompositionAfterEntityPreview() {
        TextureBinder.restoreDefaultTextureUnit();
        MinecraftReflectionCompat.setClientActiveTexture(MinecraftReflectionCompat.defaultTexUnit());
    ...
    public boolean isInventoryEntityPreview(Entity entity, double x, double y, double z) {
        ...
        return mc != null
                ...
                && guiEntityPreviewStateDepth == 0
                && entity == MinecraftReflectionCompat.player(mc)
                && isGuiPreviewPosition(x, y, z);
    }

    static boolean isGuiPreviewPosition(double x, double y, double z) {
        return Math.abs(x) < 0.01D && Math.abs(y) < 0.01D && Math.abs(z) < 0.01D;
    }

    ...
    public static void onForgeEvent(Event event) {
        if (BetterPortalsCompat.handleRenderPassEvent(event)) {
            ...
            if (shaderPackManager != null && parentWorld != null
                    && MinecraftReflectionCompat.worldProvider(parentWorld) != null) {
                shaderPackManager.restoreAfterBetterPortalsNestedRender(MinecraftReflectionCompat.providerDimension(MinecraftReflectionCompat.worldProvider(parentWorld)));
            }
        }
    }
}

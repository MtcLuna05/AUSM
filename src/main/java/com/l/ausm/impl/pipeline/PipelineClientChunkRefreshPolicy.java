...
final class PipelineClientChunkRefreshPolicy {
    ...
    static long chunkKey(int chunkX, int chunkZ) { return ((long) chunkX << 32) ^ (chunkZ & 0xFFFFFFFFL); }
    ...
    static int maxSections(int budget) { return Math.max(1, budget); }
    static int clampSectionCursor(int sectionY, int sectionCount) { return Math.max(0, Math.min(sectionY, Math.max(0, sectionCount - 1))); }
}

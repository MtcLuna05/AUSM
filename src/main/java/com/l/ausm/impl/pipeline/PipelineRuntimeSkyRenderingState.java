...
import static com.l.ausm.impl.pipeline.PipelineSkyConstants.ASTRAL_NATIVE_CONSTELLATIONS_OPTION;
import static com.l.ausm.impl.pipeline.PipelineSkyConstants.ASTRAL_NATIVE_STARS_OPTION;
import static com.l.ausm.impl.pipeline.PipelineSkyConstants.ASTRAL_SKYBOX_CLASS;
...
abstract class PipelineRuntimeDiagnosticsState2 extends PipelineRuntimeDiagnosticsState1 {
    ...
    public boolean shouldSuppressBotaniaVoidSkyBaseGeometry() {
        ...
        World world = mc != null ? MinecraftReflectionCompat.world(mc) : null;
        return world != null
                && (isPipelineActive && self().shouldUseShaderOwnedSkyOverride(world)
                || self().shouldUseCompleteOwnedSkyOverride())
                && !self().isRenderingBetterPortalsNestedView()
                && !self().isRenderingBetterPortalsRenderPass();
    }

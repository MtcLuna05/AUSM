    ...
    protected NothiriumShadowRenderer.DrawStats drawChunks(BlockRenderLayer layer, NothiriumShadowRenderer.Reflection reflection, Object pass, Iterable<?> chunks,
        ...
        try {
            ...
            for (Object chunk : chunks) {
                ...
                if (!NothiriumShadowRenderer.validDrawRange(offset, size, previousVboSize)) {
                    stats.invalidRange++;
                    continue;
                }
                ...
                self().probeLilyShadowMaterial(context, layer, chunkX, chunkY, chunkZ, offset, size, stride, pipelineStride);
                if (useChunkOffsetUniform) {
                    ...
                    NothiriumShadowRenderer.DrawProbe drawProbe = self().captureVisibleTerrainDrawProbe(layer, chunkX, chunkY, chunkZ, cameraX, cameraY, cameraZ,
                            vbo, first, count, offset, size, stride, previousVboSize, pipelineStride);
                    int query = self().beginProbeQuery(drawProbe);
                    GL11.glDrawArrays(activeDrawMode, first, count);
                    self().finishVisibleTerrainDrawProbe(drawProbe, query);
                } else {
